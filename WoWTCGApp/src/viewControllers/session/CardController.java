...
public class CardController extends javax.swing.JPanel {
    ...
    public CardController scale(Dimension newDimension, float scale){
	if(CARD_ANCHOR == CARD_ANCHOR_LEFT) setPreferredSize(new Dimension((int)(scale*(newDimension.width + 15*card.attachedCards().size())), (int)(scale*(newDimension.height))));
	else if (CARD_ANCHOR == CARD_ANCHOR_BOTTOM) setPreferredSize(new Dimension((int)(scale*(newDimension.width)), (int)(scale*(newDimension.height + 15*card.attachedCards().size()))));
	return scaleIcon(newDimension, scale);
    }
    ...
    public CardController scaleIcon(Dimension newDimension, float scale){
	if(CARD_ANCHOR == CARD_ANCHOR_LEFT) contentPanel.setBounds(0, 0, (int)(scale*newDimension.width), (int)(scale*newDimension.height));
	else if (CARD_ANCHOR == CARD_ANCHOR_BOTTOM) contentPanel.setBounds(0, getPreferredSize().height-newDimension.height, (int)(scale*newDimension.width), (int)(scale*newDimension.height));
	content.setIcon(ImageLoader.scaleImage(card.getIcon(), new Dimension((int)(newDimension.getWidth()*scale), (int)(newDimension.getHeight()*scale))));
	repaint();
